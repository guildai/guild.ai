...
`isolate-runs`: <div id="step-label"></div>
: Limit visible runs to those generated by the step parent run
  (boolean)

    By default, only runs generated by the step parent are visible to
    a step run. Set this value to `false` to consider runs generated
    outside the step parent when resolving operation dependencies for
    the step.
...
`flag-name` <div id="resource-flag-name"></div>
...
`description` <div id="resource-description"></div>
: Resource description (string)

    This value is used to annotate the resource in the Guild file. It
    is not otherwise used by Guild.

`target-path` <div id="resource-target-path"></div>
: Path under which resolved resources are located (string)

    By default, Guild creates links to resolved resource in the run
    directory. `target-path` may be used to specify a subpath to
    create links in.

    This value serves as a default for resource sources, which may
    define their own `target-path` attributes.

`target-type` <div id="resource-target-type"></div>
: How the target should be created (string)

    This value provides a default value sources that don't otherwise
    specify `target-type`. Refer to [Resource
    Sources](#resource-source-target-type) for details on this
    attribute.

`default-unpack` <div id="resource-default-unpack"></div>
...
`sources` <div id="resource-sources"></div>
...
`file` <div id="resource-source-file"></div>
...
`url` <div id="resource-source-url"></div>
: A network accessible file.

    The value for `url` types is a valid URL.

`operation` <div id="resource-source-operation"></div>
...
`config` <div id="resource-source-config"></div>
...
`module` <div id="resource-source-module"></div>
...
`name` <div id="resource-source-name"></div>
...
`target-path` <div id="resource-source-target-path"></div>
: Path under which resolved source files are created (string)

    By default, Guild creates links in the run directory. Use
    `target-path` to specify a subpath. This value overrides any
    defined by a [parent resource](#resource-target-path).

`target-type` <div id="resource-source-target-type"></div>
: How the target should be created (string)
    ...
    If this value is not specified for a source, the value for the
    [parent resource](#resource-target-type), if specified, is
    used. Otherwise the default is assumed.

    The default behavior is to create a symlink (i.e. the `link` behavior).

    !!! important
        The default value will change to `copy` in a future
        release of Guild.

    Use `copy` to ensure that resolved files become part of the run,
    rather than links. Upstream sources may change after the run,
    invalidating the run record.

    For large sources that are not likely to change, such as read-only
    data sources, use `link` to avoid the time and disk costs
    associated with copying. Note in such cases that changes to linked
    sources after the run invalidate the run record.

`sha256` <div id="resource-source-sha256"></div>
...
`unpack` <div id="resource-source-unpack"></div>
...
`select` <div id="resource-source-select"></div>
...
`select-min` and `select-max`  <div id="resource-source-select-min-max"></div>
...
`rename` <div id="resource-source-rename"></div>
...
`post-process` <div id="resource-source-post-process"></div>
...
`warn-if-empty` <div id="resource-source-warn-if-empty"></div>
...
`fail-if-empty` <div id="resource-source-fail-if-empty"></div>
...
`help` <div id="resource-source-help"></div>
