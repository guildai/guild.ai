...
We're interested in the `fine-tune` operation. Operations are run
using the [](cmd:run) command in this form:

    guild run MODEL:OPERATION [FLAG...] [DEPENDENCY...]

Let's dive in and run the `fine-tune` operation on `resnet-50`:

``` command
guild run resnet-50:fine-tune
```
...
the `fine-tune` operation:

``` command
guild run resnet-50:fine-tune --help-op
```

``` output
Usage: guild run [OPTIONS] slim-resnet-50:fine-tune [FLAG]...
...
``` command
guild run resnet-50:fine-tune dataset=custom
```
...
Guild could not complete the `fine-tune` operation because it needs
...
another operation. In this case, `fine-tune` needs images generated by
...
training and return to the `fine-tune` operation later.
